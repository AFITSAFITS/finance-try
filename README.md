...
飞书机器人会收到卡片消息，包含股票代码、信号、交易日、收盘价、涨跌幅、评分、级别、观察结论、观察仓位、执行提示、数据时效、数据来源、60 日位置、量能比、相对强度、强弱分层、资金流确认、K 线形态、参考止损、参考目标、指标类型和风险提示，比纯文本更适合在群里快速浏览。
...
扫描结果会带 `信号评分`、`信号方向`、`信号级别`、`观察结论`、`观察仓位`、`执行提示`、`数据时效`、`数据滞后天数`、`评分原因`、`风险提示`、`60日位置`、`量能比`、`20日涨幅`、`60日涨幅`、`相对强度`、`相对强度分层`、`主力净流入(亿)`、`资金流确认`、`K线形态`、`K线提示`、`参考止损`、`参考目标` 和 `风险收益比`，可以用 `--min-score` 只保留更值得观察的信号。
`观察结论` 会把评分、方向、风险提示和止损距离合成重点观察、谨慎观察、正常观察、暂不参考或风险回避。
`观察仓位` 和 `执行提示` 用来把观察结论翻译成更直观的执行上限；数据明显滞后或偏空信号会直接提示不参与。
`数据时效` 会标出行情是当日数据、最近交易日还是可能滞后；明显滞后的数据会进入风险提示并降低评分。
`相对强度` 会把股票放在本次扫描的股票池里横向比较，偏强的多头信号会加分，明显偏弱的会降权。
临时日线扫描可以选择同时检查资金流；偏多信号如果获得主力净流入支持会加分，如果资金流出会标成资金背离并提示风险。
...
命令行和每日任务会输出 `signal_summary`，快速汇总命中数、可观察数量、建议跳过数量、最高评分、观察结论分布、观察仓位分布、资金流分布、数据时效分布和相对强弱分布。
如果一次扫描虽然命中信号，但全部都提示不参与，运行记录会标成“无可观察”，避免误以为这次有可跟进机会。
每日任务会保存扫描运行记录，方便回看每次是否成功运行、命中多少、错误多少、有没有数据滞后或旧缓存兜底；运行记录会给出 `正常`、`缓存兜底`、`部分失败`、`数据滞后`、`无信号` 或 `失败` 状态。开启扫描后复盘时，运行记录也会保存复盘快照数量、复盘统计数量和复盘错误。
默认会限制单个行情源的最长等待时间，避免某个外部数据源卡住整批扫描。可以用 `AI_FINANCE_PROVIDER_TIMEOUT_SECONDS` 调整，默认 12 秒。
日线扫描会优先复用当天已抓取的本地 K 线缓存；缓存过期时会尝试刷新外部行情源，如果外部源临时不可用且本地缓存数量足够，会先使用旧缓存兜底，并在结果里标出数据来源。
...
每日任务会自动匹配已有复盘结论，默认使用 T+1；如果当前信号所属分组已经有“保留、降权、样本不足”等结论，终端和飞书通知会带上策略结论、可信度、复盘样本数和下一步动作。需要切换周期时，可以用 `--strategy-guard-horizon T+3` 或环境变量 `AI_FINANCE_STRATEGY_GUARD_HORIZON`。如果希望已降权分组不再打扰，可以加 `--mute-downgraded-strategies`，或设置 `AI_FINANCE_MUTE_DOWNGRADED_STRATEGIES=true`；事件仍会保存，只是不通知。
扫描运行记录会保存本次策略结论匹配数量和降权静音数量，方便事后解释为什么提醒数少于入库事件数。
...
AI_FINANCE_MUTE_DOWNGRADED_STRATEGIES=false
```
...
- 日线扫描会优先复用当天本地 K 线缓存；外部源临时不可用时，会用数量足够的旧缓存兜底
