...
飞书机器人会收到卡片消息，包含股票代码、信号、交易日、收盘价、涨跌幅、评分、级别、60 日位置、量能比、指标类型和风险提示，比纯文本更适合在群里快速浏览。
...
用于快速核对当前价格、涨跌幅和成交额。优先使用东方财富，失败或缺失时自动尝试腾讯，并标记行情数据是否正常。
...
返回里会包含当前价、涨跌幅、成交额、换手率、量比、数据质量和数据来源。
